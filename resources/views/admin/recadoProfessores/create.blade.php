...
<div class="card">
    <div class="card-header">
        {{ trans('global.create') }} {{ trans('cruds.recadoProfessore.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.recado-professores.store") }}" enctype="multipart/form-data">
            @csrf
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('id_remetentes') ? 'is-invalid' : '' }}" name="id_remetentes[]" id="id_remetentes" multiple>
                    @foreach($id_remetentes as $id => $id_remetente)
                        <option value="{{ $id }}" {{ in_array($id, old('id_remetentes', [])) ? 'selected' : '' }}>{{ $id_remetente }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('leu') ? 'is-invalid' : '' }}" type="number" name="leu" id="leu" value="{{ old('leu', '') }}" step="1">
                @if($errors->has('leu'))
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('id_destinatario') ? 'is-invalid' : '' }}" name="id_destinatario_id" id="id_destinatario_id">
                    @foreach($id_destinatarios as $id => $entry)
                        <option value="{{ $id }}" {{ old('id_destinatario_id') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>

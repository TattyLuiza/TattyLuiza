...
<div class="card">
    <div class="card-header">
        {{ trans('global.create') }} {{ trans('cruds.recadoTarefasProfessore.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.recado-tarefas-professores.store") }}" enctype="multipart/form-data">
            @csrf
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('id_professors') ? 'is-invalid' : '' }}" name="id_professors[]" id="id_professors" multiple>
                    @foreach($id_professors as $id => $id_professor)
                        <option value="{{ $id }}" {{ in_array($id, old('id_professors', [])) ? 'selected' : '' }}>{{ $id_professor }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <textarea class="form-control {{ $errors->has('turmas') ? 'is-invalid' : '' }}" name="turmas" id="turmas">{{ old('turmas') }}</textarea>
            ...
            <div class="form-group">
                ...
                <textarea class="form-control {{ $errors->has('alunos') ? 'is-invalid' : '' }}" name="alunos" id="alunos">{{ old('alunos') }}</textarea>
            ...
            <div class="form-group">
                ...
                <textarea class="form-control {{ $errors->has('arquivos') ? 'is-invalid' : '' }}" name="arquivos" id="arquivos">{{ old('arquivos') }}</textarea>
            ...
            <div class="form-group">
                ...
                <input class="form-control date {{ $errors->has('agendamento_at') ? 'is-invalid' : '' }}" type="text" name="agendamento_at" id="agendamento_at" value="{{ old('agendamento_at') }}">

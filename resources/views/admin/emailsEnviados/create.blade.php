...
<div class="card">
    <div class="card-header">
        {{ trans('global.create') }} {{ trans('cruds.emailsEnviado.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.emails-enviados.store") }}" enctype="multipart/form-data">
            @csrf
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('id_emails') ? 'is-invalid' : '' }}" name="id_emails[]" id="id_emails" multiple required>
                    @foreach($id_emails as $id => $id_email)
                        <option value="{{ $id }}" {{ in_array($id, old('id_emails', [])) ? 'selected' : '' }}>{{ $id_email }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('clicou') ? 'is-invalid' : '' }}" type="number" name="clicou" id="clicou" value="{{ old('clicou', '') }}" step="1" required>

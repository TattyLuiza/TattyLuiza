...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.diariosAluno.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.diarios-alunos.update", [$diariosAluno->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('diario') ? 'is-invalid' : '' }}" name="diario_id" id="diario_id" required>
                    @foreach($diarios as $id => $entry)
                        <option value="{{ $id }}" {{ (old('diario_id') ? old('diario_id') : $diariosAluno->diario->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('matricula') ? 'is-invalid' : '' }}" name="matricula_id" id="matricula_id" required>
                    @foreach($matriculas as $id => $entry)
                        <option value="{{ $id }}" {{ (old('matricula_id') ? old('matricula_id') : $diariosAluno->matricula->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('presenca') ? 'is-invalid' : '' }}" type="text" name="presenca" id="presenca" value="{{ old('presenca', $diariosAluno->presenca) }}">
            ...
            <div class="form-group">
                ...
                <textarea class="form-control ckeditor {{ $errors->has('atestado') ? 'is-invalid' : '' }}" name="atestado" id="atestado">{!! old('atestado', $diariosAluno->atestado) !!}</textarea>
            ...
            <div class="form-group">
                ...
                <textarea class="form-control ckeditor {{ $errors->has('obs') ? 'is-invalid' : '' }}" name="obs" id="obs">{!! old('obs', $diariosAluno->obs) !!}</textarea>
                @if($errors->has('obs'))
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('status') ? 'is-invalid' : '' }}" type="number" name="status" id="status" value="{{ old('status', $diariosAluno->status) }}" step="1" required>
            ...
            <div class="form-group">
                ...
                <textarea class="form-control {{ $errors->has('arquivo') ? 'is-invalid' : '' }}" name="arquivo" id="arquivo">{{ old('arquivo', $diariosAluno->arquivo) }}</textarea>
